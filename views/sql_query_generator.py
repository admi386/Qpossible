...
# Initialize state to store the last LLM response for this page
page_key = "llm_response_query_page"
if page_key not in st.session_state:
    st.session_state[page_key] = None  # Default to None for this page
...
async def generate_sql_query(user_query, metadata):
    prompt = f"User query: '{user_query}'. Generate an SQL query based on the relevant tables and columns from the database metadata: {json.dumps(metadata)}."
    response = await llm_wrapper.generate_response_async(prompt)
    
    if response:
        st.session_state[page_key] = response  # Store the response for this specific page
    else:
...
def run_async(func, *args, **kwargs):
    ...
    if loop and loop.is_running():
        thread = RunThread(func, args=args, kwargs=kwargs)
        thread.start()
        thread.join()
...
user_query = st.text_area("Enter your data to generate SQL:")
...
if st.button("Generate SQL Query"):
    ...
    if connection:
        ...
        run_async(generate_sql_query, user_query, metadata)
        connection.close()

# Display the last response for this page, if available
if st.session_state[page_key]:
    st.write("## LLM Response:")
    st.write(st.session_state[page_key])
